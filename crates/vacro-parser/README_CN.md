...
| `#(x..)`             | **具名收集**: 收集直到当前作用域结束       | `TokenStream`         | `#(rest..)`                                                           |
| `#(x?..)`            | **具名可选收集**: 可选地收集剩余 token     | `Option<TokenStream>` | `#(prefix?..)`                                                        |
| `#(x..[stop])`       | **具名终止收集**: 收集直到终止符并消费它   | `TokenStream`         | `#(prefix..[:])`                                                      |
| `#(x?..[stop])`      | **具名可选终止收集**: 终止符不存在则跳过   | `Option<TokenStream>` | `#(prefix?..[:])`                                                     |
| `#(x.. \| #(...))`   | **具名后继收集**: 收集直到后继捕获匹配     | `TokenStream`         | `#(token.. \| #(mode: OptionMode { Spaced: #{:}, Assigned: #{=} }))`  |
| `#(x?.. \| #(...))`  | **具名可选后继收集**: 后继捕获不存在则跳过 | `Option<TokenStream>` | `#(token?.. \| #(mode: OptionMode { Spaced: #{:}, Assigned: #{=} }))` |
