# Vacro

**让 Rust 过程宏开发重归简单：声明式解析库**

[](https://www.google.com/search?q=https://crates.io/crates/vacro)
[](https://www.google.com/search?q=https://docs.rs/vacro)
[](https://www.google.com/search?q=LICENSE)

## 简介

**Vacro** 是一个专为 Rust 过程宏（Procedural Macros）设计的声明式解析库。

如果你受够了使用 `syn` 时编写冗长的命令式代码（无数的 `input.parse()?`、手动的 `lookahead`、复杂的 `Punctuated` 处理），那么 **Vacro** 就是为你准备的。

**核心理念：站在巨人的肩膀上。**
Vacro 并不发明新的 AST 类型。所有的解析结果依然是标准的 `syn::Ident`、`syn::Type`、`syn::Expr` 等。我们只是提供了一种类似 `macro_rules!` 的**声明式语法**，自动生成底层的 `syn` 解析逻辑，让你的宏代码从“怎么解析”转变为“解析什么”。

## 痛点对比

假设我们要解析一个带有泛型的函数签名：`fn my_func<T, U>(a: i32) -> bool`。

### ❌ 传统写法 (Raw Syn)

为了解析这个结构，你需要编写几十行样板代码来处理泛型、括号、逗号分隔符和可选返回值：

```rust
// 传统的 syn 解析逻辑：逻辑分散，容易出错
impl Parse for MyFn {
    fn parse(input: ParseStream) -> Result<Self> {
        input.parse::<Token![fn]>()?; // 1. 吃掉关键字

        // 2. 手动处理泛型 (Peek + 解析)
        let generics = if input.peek(Token![<]) {
             input.parse::<Token![<]>()?;
             let params = Punctuated::parse_terminated(input)?;
             input.parse::<Token![>]>()?;
             Some(params)
        } else {
             None
        };

        let name: Ident = input.parse()?; // 3. 解析名字

        let content;
        parenthesized!(content in input); // 4. 处理括号
        let args: Punctuated<FnArg, Token![,]> =
            content.parse_terminated(FnArg::parse, Token![,])?;

        // 5. 处理可选的返回值
        let ret = if input.peek(Token![->]) {
            input.parse::<Token![->]>()?;
            Some(input.parse::<Type>()?)
        } else {
            None
        };

        Ok(MyFn { name, generics, args, ret })
    }
}
```

### ✅ 使用 Vacro

使用 **Vacro**，你只需要描述语法长什么样，所见即所得。

```rust
define!(MyFn:
    fn                                    // 匹配字面量
    #(?: <#(generic*[,]: GenericParam)>)  // 可选的泛型参数列表（尖括号包裹 + 逗号分隔）
    #(name: Ident)                        // 具名捕获函数名
    ( #(args*[,]: FnArg) )                // 参数列表（圆括号包裹 + 逗号分隔）
    #(?: -> #(ret: Type))                 // 可选的返回值
);
```

如果写到一行：

```rust
define!(MyFn: fn #(?: <#(generic*[,]: GenericParam)>) #(name: Ident) (#(args*[,]: FnArg)) #(?: -> #(ret: Type)));
```

一行代码，涵盖了所有复杂的解析逻辑。

## 核心宏

Vacro 提供了两个核心宏，分别用于**定义结构体**和**即时解析**。

### 1\. `define!`：定义解析结构体

如果你需要定义一个可复用的 AST 节点（即定义一个 Struct 并自动实现 `syn::parse::Parse`），请使用 `define!`。

```rust
use syn::{Ident, Type, FnArg, GenericParam, Token, parse_macro_input};
use vacro::define;

// 定义一个名为 MyFn 的结构体，它会自动实现 Parse trait
define!(MyFn:
    fn
    #(?: <#(generic*[,]: GenericParam)>)
    #(name: Ident)
    ( #(args*[,]: FnArg) )
    #(?: -> #(ret: Type))
);

// 使用方式
let my_fn: MyFn = parse_macro_input!(input as MyFn);
println!("Function name: {}", my_fn.name);
```

### 2\. `capture!`：即时流解析

如果你在现有的解析逻辑中，想要快速消费一段 Token 流，请使用 `capture!`。

#### 命名捕获 (Named Capture)

如果在模式中使用了 `name: Type` 的形式，宏会生成一个包含所有字段的结构体 `Output`。

```rust
let captured = capture!(input ->
    fn #(name: Ident) #(?: -> #(ret: Type))
)?;

// 访问字段
captured.name; // Ident
captured.ret; // Option<Type>
```

#### 行内捕获 (Inline Capture)

如果模式中没有指定名称（或只包含单个匿名捕获），宏将返回元组或单个值。

```rust
// 仅解析类型，不需要名字
let (ident, ty) = capture!(input -> #(@:Ident): #(@:Type))?;

// 访问字段
ident; // Ident
ty; // Type
```

## 核心语法表

Vacro 的 DSL 设计直觉来源于 `macro_rules!` 和正则表达式。

| 语法            | 类型         | 描述                                                                                  | 解析结果类型             | 示例                     |
| :-------------- | :----------- | :------------------------------------------------------------------------------------ | :----------------------- | :----------------------- |
| `literal`       | 字面量       | 匹配并消费 Token (如rust内的关键字与符号 `fn`, `->`或自定义关键字与符号 `miku`，`<>`) | `!`                      | `fn`, `->`, `miku`, `<>` |
| `#(x: T)`       | 具名捕获     | 捕获一个特定的 `syn` 类型                                                             | `T` (如 `Ident`, `Type`) | `#(name: Ident)`         |
| `#(x?: T)`      | 具名可选捕获 | 尝试解析，失败则跳过                                                                  | `Option<T>`              | `#(name?: Ident)`        |
| `#(x*[sep]: T)` | 具名迭代捕获 | 类似 `Punctuated`，按分隔符解析                                                       | `Punctuated<T, sep>`     | `#(args*: Ident)`        |
| `#(T)`          | 匿名捕获     | 捕获一个特定的 `syn` 类型，但仅作验证                                                 | `!`                      | `#(Ident)`               |
| `#(?: T)`       | 匿名可选捕获 | 仅作验证，失败则跳过                                                                  | `!`                      | `#(?: Ident)`            |
| `#(*[sep]: T)`  | 匿名迭代捕获 | 类似 `Punctuated`，按分隔符解析                                                       | `!`                      | `#(*[,]: Ident)`         |

# Vacro 项目开发路线图 (Roadmap)

## 📅 阶段一：夯实基础 (v0.1.x) - 当前重点

**目标：** 确保现有核心宏（`define!`, `capture!`）稳定可靠，建立完善的测试与文档体系。

### 1. 完善文档与注释 (Documentation)

- [ ] **API 文档化**：为 `Pattern`, `CaptureInput`, `Keyword` 等核心结构添加详细的 Rustdoc，确保 `docs.rs` 可读。
- [ ] **README 增强**：整合最新的中文 README，添加 `examples/` 目录，提供基础的实战案例（如解析简单的结构体和函数）。
- [ ] **错误提示优化**：优化 `syn::Error` 的生成，确保当 DSL 语法错误（如括号不匹配）时，用户能收到清晰的编译器报错，而非内部 Panic。

### 2. 完善测试体系 (Testing)

- [ ] **单元测试 (Unit Tests)**：
  - [ ] 覆盖 `inject_lookahead` 的边缘情况（递归 Group、连续 Literal 等）。
  - [ ] 测试 `Keyword` 解析器对特殊符号（`->`, `=>`, `<`）及自定义关键字的解析能力。
- [ ] **UI 测试 (Compile-fail Tests)**：
  - [ ] **集成 `trybuild`**。
  - [ ] 编写“反向测试用例”：测试当输入不符合预期类型时（例如期望 `Ident` 却给了 `LitStr`），宏能否正确拦截并报错。
- [ ] **集成测试 (Integration Tests)**：
  - [ ] 模拟真实场景，验证 `define!` 生成的结构体能否正确处理复杂的 Token 流。

---

## 🚀 阶段二：架构革新 (v0.2.x) - 核心增强

**目标：** 引入高级数据结构映射能力，解决复杂 AST 的“多态”与“聚合”问题，使 Vacro 具备处理复杂语法树的能力。

### 3. 新语法开发 (New Syntax)

#### A. 关联捕获 (Associative/Structural Capture)

_解决“结构体数组 (AoS)”问题，即一次性捕获聚合的结构，而非分散的字段列表。_

- [ ] **语法实现**：支持 `#(~name...: ...)` 语法标记聚合捕获。
- [ ] **元组支持**：实现 `#(~items*: #(@:Type) #(@:Ident))`，生成 `Vec<(Type, Ident)>`。
- [ ] **结构体支持**：支持内部具名捕获，生成匿名结构体列表。

#### B. 枚举解析 (Polymorphic Capture)

_解决“多态解析”问题，即一个位置可能是多种类型之一。_

- [ ] **语法实现**：支持 `#(name: EnumName { VariantA, VariantB })` 语法。
- [ ] **自动定义**：如果 `EnumName` 未定义，自动生成包含 `VariantA(TypeA)`, `VariantB(TypeB)` 的枚举定义。
- [ ] **分支解析**：生成基于 `input.fork()` 或 `peek` 的尝试解析逻辑，自动处理解析失败的回退（Backtracking）。

---

## 🛠️ 阶段三：生态与工具 (v0.3.x) - 开发者体验

**目标：** 提供周边工具，降低 Vacro 的学习曲线和调试成本。

### 4. 相关工具链开发 (Toolchain)

- [ ] 敬请期待

---

## License

MIT
